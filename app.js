  ...
  if (checkbox.checked) {
    if (!document.getElementById(`row-${roundId}-${playerName}`)) {
      ...
      row.innerHTML = `
        ...
        <td>
          <label><input type="radio" name="type-${roundId}-${playerName}" value="gain" checked /> Gain</label>
          <label><input type="radio" name="type-${roundId}-${playerName}" value="loss" /> Loss</label>
        </td>
      `;
...
function addRound() {
  ...
  roundDiv.innerHTML = `
    <h3>Round ${roundId + 1}</h3>
    <div class="player-checks mb-2">${checkboxes}</div>
    <table class="table table-bordered">
      <thead class="table-light"><tr><th>Player</th><th>Amount</th><th>Gain/Loss</th></tr></thead>
      <tbody id="entries-${roundId}"></tbody>
    </table>
    <div class="error text-danger" id="error-${roundId}"></div>
  `;
